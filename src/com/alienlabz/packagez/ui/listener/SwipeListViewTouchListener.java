package com.alienlabz.packagez.ui.listener;
...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	public SwipeListViewTouchListener(ListView listView, OnSwipeCallback callback) {
		...
		mAnimationTime = listView.getContext().getResources().getInteger(android.R.integer.config_longAnimTime);
		mListView = listView;
		mCallback = callback;
	}
	 ...
	 */
	public SwipeListViewTouchListener(ListView listView, OnSwipeCallback callback, boolean dismissLeft,
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		if (mViewWidth < 2) {
			mViewWidth = mListView.getWidth() + 510;
		}
		...
		case MotionEvent.ACTION_UP: {
			...
			mVelocityTracker.addMovement(motionEvent);
			mVelocityTracker.computeCurrentVelocity(1000000); // 1000 by defaut but it was too much
				...
				mDownView.animate().translationX(swipeRight ? mViewWidth : -mViewWidth).alpha(0)
						.setDuration(mAnimationTime).setListener(new AnimatorListenerAdapter() {
							...
							public void onAnimationEnd(Animator animation) {
								performSwipeAction(downView, downPosition, toTheRight, toTheRight ? dismissRight
										: dismissLeft);
							}
						});
			} else {
